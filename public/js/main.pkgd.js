...
var color = quantize$1().domain([0, 9]).range(['#ffffe5', '#f7fcb9', '#d9f0a3', '#addd8e', '#78c679', '#41ab5d', '#238443', '#006837', '#004529']);

var svg = select('body').selectAll('svg').data(sequence(2017, 2018)).enter().append('svg').attr('width', width).attr('height', height).append('g').attr('transform', 'translate(' + (width - cellSize * 53) / 2 + ',' + (height - cellSize * 7 - 1) + ')');
...
rects.append('rect').attr('width', cellSize).attr('height', cellSize).attr('x', function (d) {
  return sunday.count(year(d), d) * cellSize;
}).attr('y', function (d) {
...
svg.append('g').attr('fill', 'none').attr('stroke', '#000').selectAll('path').data(function (d) {
  return months(new Date(d, 0, 1), new Date(d + 1, 0, 1));
